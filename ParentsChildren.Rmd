...
* Teens who feel pressure to only post content that makes them look good to others, has a positive regression coefficient to him/her posting public affection towards his/her significant other. There is no significant statistical evidence to show that various teen's parent's behaviours, such as being friends with child in Facebook or other social media, have an effect on teen's public affection behaviour.  
  
* Do stalking behaviour of parents influence child's stalking behaviour on his/her significant other? For male teens, a parent monitoring child's location has a positive, statistically significant, impact on the teen exhibiting stalking behaviours.   Age has a negative (but small) impact on the teen exhibiting stalking behaviours; but its confidence level is at 90%.  For female teens, a parent checking on which websites his/her child visits, has a negative, statistically significant, impact on the teen exhibiting stalking behaviours. Perhaps the more the parent does this, the more likely the female teen feels irksome and does not do the same to her significant other.  

* On whether parent's teaching their child on appropriate behaviour is effect:  For male teens, a parent's teaching on online behaviour do not seem to affect whether teen exhibits appropriate behaviour. All regression coefficients are not statistically significant.  For female teens, a parent talking with teen about what is appropriate or inappropriate content for them to be viewing online, and what to share online, have positive, statistically significant, regression coefficients with teen's appropriate behaviour.   
...
It seems that when the parent is connected to the child (option 2), the proportion of teens who express their affection for their significant other publicly, is higher.  
...
data_specific$friends_facebook <- abs(data_specific$friends_facebook - 3) 

data_specific$check_child_profile <- survey_data$P13_C
data_specific[ is.na(data_specific$check_child_profile), ] <- NA

data_specific[ !is.na(data_specific$check_child_profile) & (data_specific$check_child_profile < 1 | data_specific$check_child_profile > 3), ] <- NA  # needs the !is.na condition otherwise an error throws up
data_specific$check_child_profile <- abs(data_specific$check_child_profile - 4) 
...
data_specific[ !is.na(data_specific$check_child_website) & (data_specific$check_child_website < 1 | data_specific$check_child_website > 3), ] <- NA  

data_specific[ !is.na(data_specific$monitor_child_location) & (data_specific$monitor_child_location < 1 | data_specific$monitor_child_location > 3), ] <- NA  

data_specific[ !is.na(data_specific$look_child_phone_records) & (data_specific$look_child_phone_records < 1 | data_specific$look_child_phone_records > 3), ] <- NA  
...
# convert from 1(YES)-2(NO)-3(does Not Apply)
#           to 0(does Not Apply)-1(NO)-2(YES)
data_specific$check_child_website <- abs(data_specific$check_child_website - 4) - 1

data_specific$monitor_child_location <- abs(data_specific$monitor_child_location - 4) - 1

data_specific$look_child_phone_records <- abs(data_specific$look_child_phone_records - 4) - 1

# convert from 1(YES)-2(NO)
#           to 1(NO)-2(YES)
data_specific$access_date_phone <- abs(data_specific$access_date_phone - 3) 

data_specific$track_date_loc <- abs(data_specific$track_date_loc - 3) 
...
For male teens, a parent monitoring child's location has a positive, statistically significant, impact on the teen exhibiting stalking behaviours.  Age has a negative (but small) impact on the teen exhibiting stalking behaviours; but its confidence level is at 90%.  
    
For female teens, a parent checking on which websites his/her child visits, has a negative, statistically significant, impact on the teen exhibiting stalking behaviours. Perhaps the more the parent does this, the more likely the female teen feels irksome and does not do the same to her significant other.  
...
# convert from 1(YES)-2(NO)
#           to 1(NO)-2(YES)
data_specific$fight_friends <- abs(data_specific$fight_friends - 3) 
data_specific$flirt_messages <- abs(data_specific$flirt_messages - 3) 
...
For female teens, a parent talking with teen about what is appropriate or inappropriate content for them to be viewing online, and what to share online, have positive, statistically significant, regression coefficients with teen's appropriate behaviour.   
